...
from MAF.algorithms.postprocessing.casual_path_tracing.lib.utils import (
    LamatrexDataset, ModelAndTokenizer, set_utils, predict_from_normal_and_noise_input, get_stopwords, make_inputs, predict_from_input    
)
from MAF.algorithms.postprocessing.casual_path_tracing.lib.causal_flow_tracer import (
    CausalFlowTracer, return_forward_method_dict
)  
from MAF.algorithms.postprocessing.casual_path_tracing.lib.search_func import (
    top_down_causal_flow_trace, get_detailed_corrupted_flow_tracer_one_block, block_initializer
)
...
class CausalPathTracing:
    ...
    def load_and_postprocess_data(self):
        dataset_loaders = {
            'lama_trex': LamatrexDataset("MAF/data/casual_path_tracing")
            }
    ...
    def load_model(self):
        ...
        self.model = ModelAndTokenizer(
            ...
            torch_dtype=(torch.float16 if "20b" in model_loaders[self.model_name] else None),
        )
    ...
    def fairness_algorithm_fit(self):
        ...
        if self.args.correct_check_first:
            ...
            self.func_time_saver.logging(self.logger, header="\n\n===============Correct Check Log===============")
            # Data Iteration
            correct_ids = []
            for li, statement in tqdm.tqdm(enumerate(self.dataset_loader), desc="Correct Check First"):
                prompt = statement["prompt"]
                y = statement["attribute"]

                flow_tracer = CausalFlowTracer(self.args)
                correctness = predict_from_normal_and_noise_input(
                    prompt, flow_tracer, y, self.logger,
                    self.model, 0.1, end_symbol=self.args.end_symbol, 
                    out_num=1, num_normal_sample=self.args.num_noise_sample, 
                    num_noise_sample=self.args.num_noise_sample, noise_type=self.args.noise_type, correct_check_only=True)
                if correctness:
                    correct_ids.append(li)
            self.logger.info("\t Correct Data Num.: {}".format(len(correct_ids)))
            np.savetxt(os.path.join(self.args.save_root, "correct_data_idx.txt"), np.asarray(correct_ids), fmt="%s")
        #---Correct Sample Check First---#
        ...
        for li, statement in tqdm.tqdm(enumerate(self.dataset_loader)):
            ...
            flow_tracer, answer, curr_total_token_num, curr_total_block_num, normal_inp = predict_from_normal_and_noise_input(
                ...
                self.model, 0.1, end_symbol = self.args.end_symbol, 
                out_num=1, num_normal_sample = self.args.num_noise_sample, 
                num_noise_sample=self.args.num_noise_sample, noise_type=self.args.noise_type)
            ...
            self.logger.info(log_line)
            self.args.curr_save_dir = os.path.join(self.args.save_result_root, "R{:04d}".format(li))
            ...
            if stop_flag is False:
                ...
                np.savetxt(os.path.join(self.args.save_inpinfo_root, "I{:06d}.txt".format(li)), success_logger, fmt="%s")
                ...
                with open(os.path.join(self.args.curr_save_dir, "C{:06d}.json".format(li)), "w") as f:
                    f.write("{\n")
    ...
    def compute_metrics(self, causal_path_result_zip):
        ...
        for li, statement in prgs:
            ...
            curr_meta_arg["mt"] = self.model
            inp = make_inputs(
                curr_meta_arg["mt"].tokenizer, [curr_meta_arg["prompt"]] * (self.args.num_noise_sample))
            ...
            with torch.no_grad():
                curr_pred, curr_p, raw_out = predict_from_input(
                    ...
                    multipred=(self.args.out_num!=1), end_symbol=self.args.end_symbol, use_mean=True, stwd_mask=flow_tracer.stwd_mask)
        ...
        return {"HitRate": sample_wise_result["metric"]["hit_rate"], "Faithfulness":sample_wise_result["metric"]["top1_faithful"]}
...
def set_parameters():
    ...
    parser.add_argument('--save_root', default="MAF/algorithms/postprocessing/casual_path_tracing/jobs/debug", type=str)
    ...
    parser.add_argument('--dataset_type', default="lama_trex", choices=['lama_trex'], help="dataset_type")

    parser.add_argument('--subset_search', default="minimality", type=str)
    parser.add_argument('--efficient_mode', default=True, help='not check counterfactual', action='store_true')
    parser.add_argument('--slightly_quiet_mode', default=True, help='slightly_quiet_mode', action='store_true')
    
    parser.add_argument('--except_stopword', default=True, help='except_stopword', action='store_true')
    parser.add_argument('--correct_check_first', default=True, help='correct_check_first', action='store_true')
...
def mitigate_cpt():
    ...
    class args:
        def __init__(self):
            self.gpu = "0"
            self.end_symbol = [".", "?"]
            self.save_root = "MAF/algorithms/postprocessing/casual_path_tracing/jobs/debug"
