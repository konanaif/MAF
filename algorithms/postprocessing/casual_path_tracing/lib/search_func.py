...
def block_initializer(args, curr_block, symbolic_name="jwon"):
    ...
    curr_block.forward = types.MethodType(
        fwd_method_dict["org_block_forward"], curr_block
    )
    curr_block.mlp.forward = types.MethodType(
        fwd_method_dict["org_mlp_forward"], curr_block.mlp
    )
    if hasattr(curr_block, "attn"):
        curr_block.attn.forward = types.MethodType(
            fwd_method_dict["org_attn_forward"], curr_block.attn
        )
    elif hasattr(curr_block, "attention"):
        curr_block.attention.forward = types.MethodType(
            fwd_method_dict["org_attn_forward"], curr_block.attention
        )
    else:
        import pdb
...
def get_detailed_corrupted_flow_tracer_one_block(
    ...
    for bidx in range(curr_meta_arg["total_block_num"]):
        ...
        if bidx < detailed_flow_tracer_idx - 1:
            curr_block.forward = types.MethodType(
                fwd_method_dict["pass_block_forward"], curr_block
            )
            ...
            curr_block.forward = types.MethodType(
                fwd_method_dict["pass_corrupted_block_forward"], curr_block
            )
    ...
    with torch.no_grad():
        _, _, _ = predict_from_input(
            curr_meta_arg["mt"].model,
            curr_meta_arg["inp"],
            multipred=(args.out_num != 1),
...
def cause_edge_classifier(args, curr_meta_arg, flow_tracer):
    ...
    for cond in ["counterfactual", "contingency"]:
        ...
        for bidx in range(curr_meta_arg["total_block_num"]):
            ...
            if bidx < curr_meta_arg["curr_block_idx"]:
                ...
                curr_block.jwon_curr_subset = tuple(
                    set().union(*flow_tracer.traced_paths[bidx])
                )
    ...
    decision_save = {
        "CF": [str(preds[0].item()), str(ps[0].item())]
        if args.efficient_mode is False
        else ["None", "None"],
...
def minimality_search(args, curr_meta_arg, flow_tracer):
    ...
    for curr_step in minsear_iter:
        ...
        buffer_cond_bool = []
        if len(subset_candidate) != 0:
            curr_meta_arg["logger"].info(
                "\t MinSearch Step: ({}/{}) -> Target Num.: {}".format(
                    curr_step, max(minsear_iter), len(subset_candidate)
                )
            )
...
def top_down_causal_flow_trace(args, curr_meta_arg, flow_tracer):

    inp = make_inputs(
        curr_meta_arg["mt"].tokenizer,
        [curr_meta_arg["prompt"]] * (args.num_noise_sample),
    )

    curr_meta_arg["inp"] = inp

    stop_flag = False
    for curr_block_idx in tqdm.tqdm(
        range(curr_meta_arg["total_block_num"] - 1, -1, -1)
    ):
        start_block_time = time.time()
        curr_meta_arg["curr_block_idx"] = curr_block_idx
        curr_meta_arg["logger"].info(
            "-----------Block Change----------- Block:{}".format(
                curr_meta_arg["curr_block_idx"]
            )
        )

        min_search_memory, stop_flag = minimality_search(
            args=args, curr_meta_arg=curr_meta_arg, flow_tracer=flow_tracer
        )
        if stop_flag:
            curr_meta_arg["logger"].info(
                "[STOP] Stopping early at block {} due to empty selection".format(
                    curr_block_idx
                )
            )
            break
        else:
            flow_tracer.traced_paths.update(
                {curr_block_idx: copy.deepcopy(min_search_memory["selected_subset"])}
            )
