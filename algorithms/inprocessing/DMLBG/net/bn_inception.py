...
class bn_inception(nn.Module):
    def __init__(self, embedding_size, pretrained = True, is_norm=True, bn_freeze = True, disent=False):
        ...
        self.disent = disent
        self.model = BNInception(embedding_size, pretrained, is_norm, disent)
        if pretrained:
#             weight = model_zoo.load_url('http://data.lip6.fr/cadene/pretrainedmodels/bn_inception-239d2248.pth')
        ...
        if self.disent == True:
            self.model.embedding = nn.Linear(self.model.num_ftrs, 2*self.model.embedding_size)
            self.model.embedding_bg = nn.Linear(self.model.num_ftrs, 2*self.model.embedding_size)
            self.model.embedding_specific = nn.Linear(2*self.model.embedding_size, 2*self.model.embedding_size)
...
class BNInception(nn.Module):
    ...
    def __init__(self, embedding_size, pretrained = True, is_norm=True, disent=True):
    ...
    def forward(self, input):
        ...
        if self.disent:
            ...
            x_emb_bg = self.embedding_bg(x)
            x_emb_specific = self.embedding_specific(x_emb)
            if self.is_norm:
